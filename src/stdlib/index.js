...
export const StdLib = {
    ...
    getModules() {
        ...
        const mapAnyToHank = (v) => {
            ...
            if (Array.isArray(v))
                return { type: ValueType.Array, value: v.map(i => mapAnyToHank(i)) };
            if (typeof v === 'object') {
                const map = new Map();
                for (const [k, val] of Object.entries(v))
                    map.set(k, mapAnyToHank(val));
        ...
        const mapHankToAny = (v) => {
            switch (v.type) {
                ...
                case ValueType.String: return v.value;
                case ValueType.Array: return v.value.map((i) => mapHankToAny(i));
                case ValueType.Object:
                    const obj = {};
                    v.value.forEach((val, k) => { obj[k] = mapHankToAny(val); });
                    return obj;
        ...
        return {
            ...
            math: {
                add: (args) => ({ type: ValueType.Number, value: args.reduce((sum, a) => sum + (a.type === ValueType.Number ? a.value : 0), 0) }),
                sub: (args) => (args.length < 2 || args[0].type !== ValueType.Number || args[1].type !== ValueType.Number) ? { type: ValueType.Void } : { type: ValueType.Number, value: args[0].value - args[1].value },
                mul: (args) => (args.length === 0) ? { type: ValueType.Number, value: 0 } : { type: ValueType.Number, value: args.reduce((res, a) => res * (a.type === ValueType.Number ? a.value : 1), 1) },
                div: (args) => (args.length < 2 || args[1].type !== ValueType.Number || args[1].value === 0) ? { type: ValueType.Void } : { type: ValueType.Number, value: args[0].value / args[1].value },
                gt: (args) => (args.length < 2 || args[0].type !== ValueType.Number || args[1].type !== ValueType.Number) ? { type: ValueType.Void } : (args[0].value > args[1].value ? { type: ValueType.Number, value: 1 } : { type: ValueType.Void }),
                lt: (args) => (args.length < 2 || args[0].type !== ValueType.Number || args[1].type !== ValueType.Number) ? { type: ValueType.Void } : (args[0].value < args[1].value ? { type: ValueType.Number, value: 1 } : { type: ValueType.Void }),
            ...
            arr: {
                ...
                get: (args) => (args.length < 2 || args[0].type !== ValueType.Array || args[1].type !== ValueType.Number) ? { type: ValueType.Void } : (args[0].value[args[1].value] || { type: ValueType.Void }),
                push: (args) => { if (args.length >= 2 && args[0].type === ValueType.Array)
                    args[0].value.push(args[1]); return { type: ValueType.Void }; },
                    ...
                    if (args.length >= 2 && args[0].type === ValueType.Array && args[1].type === ValueType.Task) {
                        ...
                        items.forEach((item, idx) => {
                            ...
                            if (task.type === ValueType.Task && !task.task.isNative) {
                                if (callArgs.length > task.task.params.length)
                                    callArgs.splice(task.task.params.length);
                            }
            ...
            obj: {
                get: (args) => (args.length >= 2 && args[0].type === ValueType.Object) ? (args[0].value.get(valToString(args[1])) || { type: ValueType.Void }) : { type: ValueType.Void },
            ...
            json: {
                ...
                stringify: (args) => {
                    if (args.length === 0)
                        return { type: ValueType.Void };
                    const checkOpaque = (val) => {
                        if (val.type === ValueType.Opaque)
                            return true;
                        if (val.type === ValueType.Array)
                            return val.value.some((i) => checkOpaque(i));
                        if (val.type === ValueType.Object)
                            return Array.from(val.value.values()).some((v) => checkOpaque(v));
                        return false;
                    };
                    if (checkOpaque(args[0]))
                        return { type: ValueType.Void };
                    try {
